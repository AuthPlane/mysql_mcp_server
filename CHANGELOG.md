...
- **MCP resources bypassed the authorization model.** `list_resources` and `read_resource` are a separate MCP primitive and never pass through the tool dispatcher, so they had neither a scope requirement nor the read/write split: reading `mysql://<table>/data` ran `SELECT * FROM <table> LIMIT 100` on the **read-write** account for any authenticated caller, including one holding no scope at all — while `read_query` ran the same statement, scope-gated, on the `SELECT`-only account. Both now require the read scope and use the read-only account.
