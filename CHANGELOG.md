...
### Fixed
- **`MYSQL_MAX_ROWS` failed on exactly the queries it exists for.** `fetchmany(max_rows + 1)` left the remainder of the result set on the connection, and mysql-connector raises `Unread result found` when a cursor with pending rows is closed — so a query that exceeded the cap failed outright instead of returning a truncated answer. Queries under the cap were unaffected, which is why it survived the mocked tests. The remainder is now drained in batches, and `test_sql_boundary.py` covers it against a live server.
- **A rejected MySQL credential was reported as a missing scope.** `1045 ER_ACCESS_DENIED_ERROR` reads like the other access-denied errnos but MySQL raises it when credentials are refused, at connect time. While it was in `DENIAL_ERRNOS`, a wrong `MYSQL_PASSWORD` reached the caller as "this statement is not permitted … a token carrying the write scope is required" — an authorization problem that does not exist, while the real one went unmentioned. It is now handled as the server misconfiguration it is, with a message that points at the server's environment and still does not name the account or host.
- **The privilege-denial message no longer claims a connection the statement may not have run on.** The same errnos arrive on the read-write connection whenever the deployed account simply lacks a privilege; there the text now says so instead of mentioning read-only privileges and the write scope.
...
- **`execute_sql` is scope-aware, and remains the only SQL tool.** The caller's scope selects the MySQL account that runs the statement, so a read-scoped caller reaches the database as the `SELECT`-only account and MySQL refuses anything that writes. A tool accepting arbitrary SQL can therefore be authorized precisely -- by the database's grants rather than by this process classifying the statement -- and needs no deprecation. Existing configurations that call `execute_sql` are unaffected.
  - An earlier iteration of this branch added `read_query` and `write_query` alongside it, each gated on a different scope. They were removed before release: once the connection is chosen by scope, their gate proved nothing the connection was not already proving, and having them next to an `execute_sql` whose gate is deliberately empty made the authorization model read as though the arbitrary-SQL tool were the unprotected one. `get_schema_info` and `get_table_sample` keep their read-scope gate, since they take a table name rather than SQL.
- **Read/write separation enforced by MySQL.** With `MYSQL_RO_USER` / `MYSQL_RO_PASSWORD` set, a read-scoped call connects as a `SELECT`-only account, so a write is refused by the database rather than by this process reading the SQL. Grants are verified at startup and the server refuses to start if that account can write.
...
### Fixed
- **An authorization-server outage was reported to clients as a bad token.** The SDK's `http_status()` distinguishes four outcomes; the translation layer kept one bit of it, mapping 403 to a scope failure and *everything else* to `401 invalid_token`. So when Authplane became unreachable or the SDK's circuit breaker opened — `JWKSFetchError`, `MetadataFetchError`, `CircuitOpenError`, all 503 — every client was told its credential was bad. A conforming client then discards a working token and re-authenticates against the server that is already down, and with `MCP_AUTH_MAX_AUTH_FAILURES` set this server throttled it for doing so. All four statuses are now carried through: 503 when the AS cannot participate in validation, 500 for an internal fault, 401 only for a credential the client can actually fix. A 5xx carries no `WWW-Authenticate` challenge (the challenge is the invitation to re-authenticate) and does not count against the failure throttle; a 503 carries `Retry-After`.
- **Scope was decided by the token that opened the stream, not the one that sent the call.** The check ran in the tool layer against a contextvar bound on `GET /sse`, so a call arriving on a narrower token for the same subject was authorized against the wider one. Down-scoping therefore had no effect — handing a sub-agent a read-only token did not stop it writing if it shared a session with a read-write one — and the `tool_call_authorized` record named a different `jti` than the scope decision had used. The identity is now resolved through the MCP request id, which is the one thing shared between the POST that delivered the frame and the task that executes it. It cannot be released when the POST returns, since the transport answers 202 before the handler runs, so the tool layer releases it and the table is capped for frames that never reach a handler.
- **MCP resources bypassed the authorization model.** `list_resources` and `read_resource` are a separate MCP primitive and never pass through the tool dispatcher, so they had neither a scope requirement nor the read/write split: reading `mysql://<table>/data` ran `SELECT * FROM <table> LIMIT 100` on the **read-write** account for any authenticated caller, including one holding no scope at all — while a scope-gated tool call ran the same statement on the `SELECT`-only account. Both now require the read scope and use the read-only account.
- **A non-root `AUTHPLANE_RESOURCE` advertised a discovery document that 404'd.** The route was registered at a fixed `/.well-known/oauth-protected-resource` while the challenge URL came from the SDK's `prm_url()`, which per RFC 9728 §3 appends the resource's path. With `AUTHPLANE_RESOURCE=https://host/mysql` the 401 pointed clients at `/.well-known/oauth-protected-resource/mysql`, which no route served, and discovery failed with nothing in the log. Both the route and the middleware's public-path exemption are now derived from the verifier's own `metadata_url()`, so they cannot disagree. Invisible in every test and live run, all of which used a root resource.
- **Session bindings were never released.** `forget()` existed and was never called, so the table only grew; on a long-lived deployment it drifted towards its 4,096-entry cap and then began evicting entries for sessions that were still open — and an evicted session is no longer subject-checked, which is the one thing the table exists to prevent. A binding is now released when its stream closes, matching the transport's own session lifetime.
- **A DPoP `htu` was built from the percent-decoded path.** `scope["path"]` is decoded by ASGI, but the client signs `htu` over the on-wire target, so a path containing `%2F` would fail proof verification here while succeeding against the TypeScript sibling. `scope["raw_path"]` is now used, with the decoded path as a fallback for servers that omit it. Latent while the only paths are `/sse` and `/messages/`.
- **The read-only account was only verified when auth was enabled.** The grant check sat inside the SSE server's `if auth_settings.enabled:` block, but `get_db_config(read_only=True)` honours `MYSQL_RO_USER` unconditionally — including over stdio, which never reaches that code at all. An operator running stdio with a misprovisioned read-only account got the weaker posture with neither the startup check nor the warning. It now runs in `main()`, ahead of the transport split, because it is a property of the database configuration rather than of the auth configuration.
...
### Changed
- **`authplane-sdk` pinned to 0.4.0** (from 0.2.0, via 0.3.0). No source change was needed at either step. The public API this integration depends on -- `AuthplaneClient.resource()`, `resource.verify()`, `prm_url()`, `prm_response()`, `http_status()`, `InboundDPoPOptions` -- is unchanged across all three, and the suite passes unchanged. Note the hermetic suite exercises the fake verifier: the tests that drive the real SDK against a live authorization server (`test_authplane_live`, `test_authplane_e2e`) skip without one, so a live run is what confirms a version bump end to end.
- **The SQL statement classifier has been removed, and `MYSQL_RO_USER` is now required.** The read path is read-only because it connects as an account that cannot write, not because this server parses statements. Measured against MySQL 8.4, the previous fallback -- `START TRANSACTION READ ONLY` on the read-write account -- refuses `INSERT`/`UPDATE`/`DELETE` (1792) but lets `CREATE`, `DROP`, `ALTER`, `TRUNCATE` and `RENAME` through, because DDL commits implicitly and ends the transaction. The classifier was the only thing covering that path, and a hand-written SQL classifier is exactly what should not be load-bearing. The server now refuses to start without the read-only account, with an error naming the `CREATE USER` / `GRANT SELECT` needed. ~550 lines of classifier and its tests are gone; `sqlguard.py` keeps only the mapping from MySQL denial errnos to a clean tool error.
- **`execute_sql` is authorized by connection rather than by a scope gate.** An earlier iteration of this branch deprecated it and required the write scope, on the reasoning that arbitrary SQL cannot be authorized precisely. That reasoning assumed a single connection. With the caller's scope selecting the account, the database enforces the split and the tool needs no deprecation: write scope routes to the read-write account, read scope alone to the `SELECT`-only account, and a caller holding neither is refused before the database is reached. Statement classification was then removed entirely, and with it the `read_query`/`write_query` pair it justified.
- **DPoP proof algorithms are configured separately** from token signing algorithms, via `MCP_AUTH_DPOP_ALGORITHMS` (defaulting to `AUTHPLANE_ALLOWED_ALGORITHMS`). One variable drove both, so proofs could not be restricted to `ES256` while `RS256`-signed tokens stayed acceptable — a reasonable posture, since the authorization server's algorithm is a deployment fact and the client's is a policy choice.
...
### Security
- **What a `SELECT`-only grant still permits was measured, not assumed:** `INTO OUTFILE` and `INTO DUMPFILE` are refused (1227) and `LOAD_FILE()` returns `NULL` without the `FILE` privilege; `FOR UPDATE` is refused (1142). `SLEEP()` and `BENCHMARK()` do run — resource consumption rather than a privilege violation, bounded by `MYSQL_STATEMENT_TIMEOUT_MS` and `MYSQL_MAX_ROWS` rather than by parsing.
