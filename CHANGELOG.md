...
### Security
- **What a `SELECT`-only grant still permits was measured, not assumed:** `INTO OUTFILE` and `INTO DUMPFILE` are refused (1227) and `LOAD_FILE()` returns `NULL` without the `FILE` privilege; `FOR UPDATE` is refused (1142). `SLEEP()`, `BENCHMARK()` and `GET_LOCK()` all run — resource consumption rather than a privilege violation, and bounded by `MYSQL_STATEMENT_TIMEOUT_MS` and `MYSQL_MAX_ROWS` rather than by parsing. With one exception: the timeout cuts `SLEEP()` and `BENCHMARK()` short and bounds how long a caller blocks waiting on a lock (`3024 ER_QUERY_TIMEOUT`), but a lock `GET_LOCK()` acquires is held until it is released or the session ends, so that one outlives its statement.
